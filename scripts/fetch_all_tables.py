...
Usage: python3 scripts/fetch_all_tables.py [--output-mode print|store] [--bucket-num 4] [--workers 8]
"""
...
def main():
    parser = argparse.ArgumentParser(description="Parallel fetch for all HyBench tables")
    parser.add_argument(
        "--output-mode",
        choices=["print", "store"],
        default="store",
    ...
    parser.add_argument("--schema", default=SCHEMA, help=f"Schema name (default: {SCHEMA})")
    parser.add_argument(
        "--tables", nargs="+", default=TABLES, help="Specific tables to fetch (default: all)"
    )
    ...
    print("=" * 70)
    print("Parallel HyBench Table Fetch")
    ...
    with ThreadPoolExecutor(max_workers=args.workers) as executor:
        futures = {
            executor.submit(
                fetch_table, table, args.schema, args.output_mode, bucket_id, project_root
            ): (table, bucket_id)
